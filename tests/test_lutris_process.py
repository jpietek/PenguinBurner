...
def test_a_game_is_started_by_its_database_id_through_the_lutris_cli(
    ...
    monkeypatch.setattr(host.subprocess, "Popen", _Popen)
...
def test_a_non_numeric_id_never_reaches_the_command_line(monkeypatch) -> None:
    ...
    called: list[object] = []
    monkeypatch.setattr(
        host.subprocess, "Popen", lambda *a, **k: called.append(a)
    )
...
def _pgrep(monkeypatch, *, stdout: str, returncode: int = 0):
    ...
    monkeypatch.setattr(host, "running_in_flatpak", lambda: False)
    monkeypatch.setattr(host.subprocess, "run", lambda *a, **k: result)
...
def test_stopping_signals_the_wrapper_rather_than_the_game(monkeypatch) -> None:
    ...
    monkeypatch.setattr(host.os, "kill", lambda pid, sig: sent.append((pid, sig)))
...
def test_stopping_a_pid_that_is_gone_is_a_failure_not_a_crash(monkeypatch) -> None:
    ...
    monkeypatch.setattr(host.os, "kill", _gone)
